"""
Species Classification - ResNet-101 Transfer Learning

Uses Excel lookup table to assign species labels based on slide names.
Trains with k-fold cross-validation and transfer learning.

18 species classes from your training data.
"""
...
CONFIG = {
    ...
    # Cross-validation
    'n_folds': 5,  # Set to 1 for simple train/val split
}
...
class SpeciesResNet(nn.Module):
    """ResNet-101 for species classification."""
    ...
    def unfreeze_deep_layers(self, num_layers=2):
        """Unfreeze last few ResNet layers."""
        ...
        for epoch in range(self.config['epochs_frozen']):
            ...
            if val_acc > best_acc:
                ...
                best_model_path = os.path.join(self.config['model_dir'], 
                                               f'species_resnet_best_{timestamp}.pth')
...
class SpeciesClassifier:
    """Use trained model for species prediction."""
...
if __name__ == "__main__":
    print("=" * 60)
    print("SPECIES CLASSIFIER - ResNet-101")
    print("=" * 60)
...
1. TRAIN:
   ...
   from species_classifier import SpeciesClassifierTrainer, CONFIG
   
   trainer = SpeciesClassifierTrainer(CONFIG)
   trainer.train()
...
2. PREDICT:
   ...
   classifier = SpeciesClassifier('models/species_resnet_best_XXXXXX.pth')
