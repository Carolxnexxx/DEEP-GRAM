...
class SpeciesDenseNet201(nn.Module):
    ...
    def unfreeze_deep_layers(self, num_blocks=2):
        ...
        print(f"Unfroze last {num_blocks} dense blocks for fine-tuning.")
...
class SpeciesDenseNet201Trainer:
    ...
    def _final_evaluation(self, model, loader, true_labels):
        ...
        plt.ylabel('True')
        plt.title('Species Confusion Matrix (DenseNet-201)')
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        
        save_path = os.path.join(self.config['model_dir'], 'species_densenet201_confusion_matrix.png')
    ...
    def plot_history(self):
        ...
        save_path = os.path.join(self.config['model_dir'], 'species_densenet201_training_history.png')
        plt.savefig(save_path, dpi=150)
...
class SpeciesDenseNet201Classifier:
    ...
    def __init__(self, model_path, image_size=(224, 224)):
        ...
        self.model = SpeciesDenseNet201(self.num_classes, pretrained=False)
        ...
        print(f"Loaded DenseNet-201 model: {model_path}")
